/**
 * Poster Maker — text→image agent for the Blocks network.
 *
 * First provider-backed handler in the foundation: every task spends
 * real money, so inputs are length-capped BEFORE the provider is
 * called and the runtime cap (agent-card.json maxRunningTimeSec) is
 * kept tight.
 */
...
export default async function handler(
  ...
  const model = process.env.POSTER_IMAGE_MODEL ?? 'gpt-image-1';
  const size = process.env.POSTER_IMAGE_SIZE ?? '1024x1024';
  ctx?.reportStatus(`Generating poster with ${model}...`);

  const body: Record<string, unknown> = {
    model,
    prompt: `A clean, striking poster illustrating: ${text}`,
    n: 1,
    size,
  };
  if (model.startsWith('dall-e')) {
    ...
    body.quality = process.env.POSTER_IMAGE_QUALITY ?? 'low';
  }
  ...
  ctx?.reportStatus(`Poster ready (${png.byteLength} bytes).`);
